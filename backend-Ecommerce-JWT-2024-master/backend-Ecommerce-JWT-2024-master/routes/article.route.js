...
router.get('/cat/:categorieID',async(req, res)=>{
    try {
        const art = await Article.find({ categorieID: req.params.categorieID}).exec();
...
router.get('/',async (req, res )=> {
    try {
      const articles = await Article.find({}, null, {sort: {'_id': -1}}).populate("categorieID").exec();
...
router.post('/', async (req, res) =>  { 
 ...
 const nouvarticle = new Article(req.body)
    try {
        ...
        const articles = await Article.findById(response._id).populate("categorieID").exec();
...
router.put('/:articleId', async (req, res)=> {
   try {
    ...
    const articles = await Article.findById(art._id).populate("categorieID").exec();
    console.log(articles)
    res.status(200).json(articles);
    } catch (error) {
    res.status(404).json({ message: error.message });
